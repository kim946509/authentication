...
class GrantAdminApiTest {
    ...
    void grantAdmin_success() throws Exception {
        ...
        // when
        MockHttpServletResponse response = mockMvc.perform(patch("/admin/users/{username}/roles", targetUser)
    ...
    void grantAdmin_fail_userNotFound() throws Exception {
        ...
        // when
        MockHttpServletResponse response = mockMvc.perform(patch("/admin/users/{username}/roles", "notExistUser")
                        .header("Authorization", "Bearer " + accessToken))
                .andReturn()
                .getResponse();
